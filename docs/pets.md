...
## Edge-snap hiding (feather strip)

Dragging a pet flush against the **left or right** work-area edge hides it
behind the edge: the window slides off-screen until only a feather strip
remains, and clicking or dragging the strip brings the pet back.

Behavior and files:

- **Trigger** — `detectSnapEdge` (`display.ts`) measures distance from the
  pet's *visible sprite* (not the transparent window rect, which is 74 px wider
  than the 192 px sprite) plus the drag cursor's screen X. Whichever is closer
  wins; threshold is `petSnapThresholdPx` (20 px). A sprite whose center has
  already crossed the edge counts as an unambiguous hide gesture. Top/bottom
  edges never snap.
- **Hidden position** — `computeSnappedHiddenPosition` slides the window so
  only `featherStripSize` (28×100, scaled with the pet) remains flush with the
  edge; the strip element is anchored to the window edge (`left:0`/`right:0`),
  so a flush window edge means a flush feather strip.
- **Drag-end hook** — `pet-window.ts` `handleDragEnd` uses the **raw**
  `window.getPosition()` for the snap check: the clamping helpers would pull
  the window back inside the work area and erase the flush state. Restore
  position is the pre-snap raw position.
- **State** — `snappedPetWindows` (WeakMap) tracks snapped windows;
  `isPetWindowSnapped` lets `default-pet-controller.ts` skip persisting the
  hidden coordinates and skip reclamping while snapped. `readWindowPosition`
  returns the raw position for snapped windows.
- **Renderer** — hidden state sets `html[data-snap-state="hidden"]` +
  `data-snap-edge` (via `pet-preload.cjs`); CSS hides the sprite, shows a
  feather strip (`assets/feather-strip.webp`, generated by
  `scripts/generate-feather-assets.mjs`) with a 6-frame sway animation, sized
  proportionally to the pet scale. The strip participates in hit-testing and
  drag (`pet-preload.cjs` interactive selectors). A click restores immediately;
  dragging starts only after the pointer moves beyond the click threshold.
- **Custom layouts** — validated frame dimensions drive the hitbox, bubble
  anchor, Linux shape, and edge detection. Oversized frames are scaled down to
  fit the fixed transparent window, and declared rows/columns must match the
  actual WebP dimensions. Per-state duration and finite iteration overrides also
  drive the main-process reaction cleanup timer.

### Codex desktop pet coexistence

While the Codex desktop app is showing its own pet, the OpenPets default pet
steps aside (`applyCodexPresence` in `default-pet-controller.ts`). Detection is
a 3 s poll (`codex-presence.ts`): on Windows, `tasklist /v` rows whose image is
`ChatGPT.exe`/`codex.exe` with window title exactly `Codex` count as visible;
other platforms leave coexistence detection disabled until a reliable
window-level detector exists. Suppression is a display gate: show requests are
deferred while Codex is present, a later explicit hide cancels restoration, and
the user's launch preference is untouched. A minimized Codex window still
counts as visible.
