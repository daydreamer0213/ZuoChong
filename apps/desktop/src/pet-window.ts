...
import { applySpriteLayoutOverride, defaultPetSprite, deriveCodexSpriteLayout, getSpriteAnimationDurationMs, motionToSpriteState, resolveReactionSpriteState, spriteLayoutMatchesImage, type PetMotionState, type PetSpriteLayout, type UniversalSpriteState } from "./reaction-animation-mapping.js";
...
export function getTransientReactionAnimationMs(display: PetTransientDisplay, layout: PetSpriteLayout = defaultPetSprite): number | null {
  ...
  return getSpriteAnimationDurationMs(layout, state);
}
...
async function createInstalledPetRender(petId: string, displayName: string, paused: boolean, display: PetTransientDisplay | null, scale: PetScaleValue, badge: PetStatusBadgeReaction | null, cachePrefix: string, dismissToken?: string, pluginBubbles: PetPluginBubbles | null = null): Promise<PetContentRender> {
  ...
  const reactionState = getReactionSpriteState(display?.reaction);
  const stateRows = layout.states;

  return {
    cacheKey: `${cachePrefix}:${paused}:${effectiveScale}:${layout.frameWidth}x${layout.frameHeight}:${layout.rows}x${layout.columns}:${spritesheet.mtimeMs}:${spritesheet.size}:${getActiveLocale()}`,
    bodyHtml,
    reactionState,
    spriteMetrics,
    html: `<!doctype html>
      <html lang="${getActiveLocaleLang()}" data-reaction-state="${reactionState}" data-motion-state="idle" data-native-pet-drag="${shouldUseWaylandNativePetDrag() ? "wayland" : "manual"}">
        <head>
          ...
          <style>
            ...
            ${createSpriteStateCss(".installed-sprite", layout)}
            @keyframes pet-frames {
              from { background-position: 0 var(--sprite-row-y); }
...
function createSpriteStateCss(selector: ".sprite" | ".installed-sprite", layout: PetSpriteLayout = defaultPetSprite): string {
  const reactionRules = Object.keys(layout.states).map((state) => createSpriteRule(`html[data-reaction-state="${state}"] ${selector}`, state as UniversalSpriteState, layout));
  const motionRules = (Object.entries(motionToSpriteState) as Array<[PetMotionState, UniversalSpriteState]>)
    ...
    .map(([motion, state]) => createSpriteRule(`html[data-motion-state="${motion}"] ${selector}`, state, layout));
  return [...reactionRules, ...motionRules].join("\n");
}

function createSpriteRule(selector: string, state: UniversalSpriteState, layout: PetSpriteLayout = defaultPetSprite): string {
  const row = layout.states[state];
