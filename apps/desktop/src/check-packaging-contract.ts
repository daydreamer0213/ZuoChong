...
assert.match(mainSource, /use-mock-keychain[\s\S]*?process\.platform === "linux"[\s\S]*?appendSwitch\("password-store", "gnome-libsecret"\)/, "Linux desktop must keep Electron safeStorage encryption available by selecting the gnome-libsecret keyring backend; forcing basic_text makes plugin secret saves fail with 'Secret storage encryption is unavailable on this system'.");
